...
public class Card extends Actor
{
    ...
    public void act() 
    {
        CardTable b = (CardTable)getWorld(); 
        if(Greenfoot.mousePressed(this)){
            if (b.getHand().contains(this) == false){
                String a= Greenfoot.ask("Add at which index in the hand?");
                int response = Integer.parseInt(a); 
                if (a.length()== 0){
                    ((CardTable)getWorld()).getHand().add(this);
                }
                else if (a.length()!= 0 && b.getHand().size()>= response){
                   ((CardTable)getWorld()).getHand().add(response,this);
                }
            }
            else{
                b.getHand().remove(this); 
            }
        }
